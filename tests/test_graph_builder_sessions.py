...
from lg_adk.sessions.session_manager import AsyncSessionManager, EnhancedSessionManager
...
class TestGraphBuilderSessions(unittest.TestCase):
    """Tests for GraphBuilder's session management integration."""

    def test_run_with_new_session(self, graph_builder, enhanced_session_manager) -> None:
        ...
        self.assertIn(session_id, enhanced_session_manager.session_metadata)
        ...
        for key, value in metadata.items():
            self.assertIn(key, session_metadata)
            self.assertEqual(session_metadata[key], value)

        # Verify interaction was tracked
        analytics = enhanced_session_manager.get_session_analytics(session_id)
        self.assertIsNotNone(analytics)
        self.assertGreater(analytics["message_count"], 0)

    def test_run_with_existing_session(self, graph_builder, enhanced_session_manager) -> None:
    ...
    def test_configure_langgraph_session_handling(
        self,
        mock_compile,
        _mock_add_node,
        graph_builder,
    ...
    @pytest.mark.asyncio
    async def test_arun_with_async_session_manager(self, mock_agent) -> None:
        ...
        self.assertIn("session_id", result)
        session_id = result["session_id"]
        self.assertIn(session_id, async_session_manager.session_manager.session_metadata)
