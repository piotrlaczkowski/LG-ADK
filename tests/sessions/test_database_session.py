...
class TestDatabaseSessionManager(unittest.TestCase):
    ...
    def test_basic_functionality(self, db_session_manager) -> None:
        ...
        self.assertTrue(db_session_manager.db_manager.exists("sessions", session_id))
        ...
        self.assertFalse(db_session_manager.db_manager.exists("sessions", session_id))

    def test_session_persistence(self, mock_db) -> None:
        ...
        self.assertEqual(session.session_id, session_id)
        self.assertEqual(session.metadata.get("persistent"), True)

    def test_metadata_management(self, db_session_manager) -> None:
        ...
        self.assertEqual(stored_session.metadata, expected_metadata)

    def test_session_timeout(self, db_session_manager) -> None:
    ...
    def test_session_with_user_id(self, db_session_manager) -> None:
    ...
    def test_tracking_interactions(self, db_session_manager) -> None:
        ...
        self.assertEqual(session.interactions, 1)
        self.assertEqual(session.total_tokens_in, 10)
        self.assertEqual(session.total_tokens_out, 5)

        # Verify interaction data is stored in database
        stored_session = db_session_manager.db_manager.get("sessions", session_id)
        self.assertEqual(stored_session.interactions, 1)
        self.assertEqual(stored_session.total_tokens_in, 10)
        self.assertEqual(stored_session.total_tokens_out, 5)

    def test_multiple_interactions(self, db_session_manager) -> None:
        ...
        self.assertEqual(session.total_tokens_in, total_tokens_in)
        self.assertEqual(session.total_tokens_out, total_tokens_out)
        ...
        # Verify interaction data is stored in database
        stored_session = db_session_manager.db_manager.get("sessions", session_id)
        self.assertEqual(stored_session.interactions, num_interactions)
        self.assertEqual(stored_session.total_tokens_in, total_tokens_in)
        self.assertEqual(stored_session.total_tokens_out, total_tokens_out)

    def test_end_nonexistent_session(self, db_session_manager) -> None:
    ...
    def test_cleanup_with_no_expired_sessions(self, db_session_manager) -> None:
    ...
    def test_serialization_and_deserialization(self, db_session_manager) -> None:
        ...
        self.assertEqual(stored_session.user_id, user_id)
        self.assertEqual(stored_session.metadata, metadata)
        self.assertTrue("nested" in stored_session.metadata)
        self.assertEqual(stored_session.metadata["nested"]["field1"], "value1")

    def test_database_failure_handling(self, mock_db) -> None:
    ...
    def test_multiple_cleanup_runs(self, db_session_manager) -> None:
        ...
        self.assertTrue(db_session_manager.db_manager.exists("sessions", medium_timeout_id))
        self.assertTrue(db_session_manager.db_manager.exists("sessions", long_timeout_id))
