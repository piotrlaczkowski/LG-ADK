...
class TestAsyncSessionManager:
    ...
    @pytest.mark.asyncio
    async def test_basic_functionality(self, async_session_manager):
    ...
    @pytest.mark.asyncio
    async def test_metadata_management(self, async_session_manager):
    ...
    @pytest.mark.asyncio
    async def test_session_timeout(self, async_session_manager):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_session_operations(self, async_session_manager):
        """Test performing multiple async operations concurrently."""
        # Create multiple sessions concurrently
        num_sessions = 10
        create_tasks = [
            create_session_async(async_session_manager)
        ...
        update_tasks = [
            update_session_async(async_session_manager, session_id)
        ...
        get_tasks = [
            get_session_async(async_session_manager, session_id)
            for session_id in session_ids
        ]
        ...
        end_tasks = [
            end_session_async(async_session_manager, session_id)
            for session_id in session_ids
        ]
    ...
    @pytest.mark.asyncio
    async def test_async_metadata_updates(self, async_session_manager):
        ...
        assert final_session.metadata["counter"] == num_updates
    
    @pytest.mark.asyncio
    async def test_parallel_metadata_updates(self, async_session_manager):
        ...
        # Verify final tags
        final_session = await get_session_async(async_session_manager, session_id)
        assert len(final_session.metadata["tags"]) == num_updates
        
        # All tags should be in the final list (order may vary)
        for tag in added_tags:
            assert tag in final_session.metadata["tags"]
    
    @pytest.mark.asyncio
    async def test_session_creation_with_user_id(self, async_session_manager):
    ...
    @pytest.mark.asyncio
    async def test_async_session_interactions(self, async_session_manager):
        ...
        assert session.total_tokens_in == sum(10 * (i + 1) for i in range(num_interactions))
        assert session.total_tokens_out == sum(5 * (i + 1) for i in range(num_interactions))
        
        # Verify timestamps
        assert session.created_at < session.last_interaction_at
    
    @pytest.mark.asyncio
    async def test_end_nonexistent_session(self, async_session_manager):
    ...
    @pytest.mark.asyncio
    async def test_cleanup_with_no_expired_sessions(self, async_session_manager):
