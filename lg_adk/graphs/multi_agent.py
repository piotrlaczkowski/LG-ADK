...
class Message(BaseModel):
    ...
    role: str = Field(..., description="Role of the message sender (system, user, assistant, researcher, writer, critic)")
    content: str = Field(..., description="Content of the message")
    agent: Optional[str] = Field(None, description="Agent that created this message")
...
class Task(BaseModel):
    ...
    assigned_to: Optional[str] = Field(None, description="Agent assigned to this task")
    result: Optional[str] = Field(None, description="Result of the task once completed")
...
def create_multi_agent_graph() -> Graph:
    ...
    def research_topic(state: MultiAgentState) -> MultiAgentState:
        ...
        for task in tasks:
            if task["assigned_to"] == "researcher":
                ...
                writing_task = Task(
                    description=f"Write content based on the research findings",
                    status="pending",
                    assigned_to="writer"
                )
    ...
    def write_content(state: MultiAgentState) -> MultiAgentState:
        ...
        research_task = next((task for task in tasks if task["assigned_to"] == "researcher" and task["status"] == "completed"), None)
    ...
    def critique_content(state: MultiAgentState) -> MultiAgentState:
        ...
        writing_task = next((task for task in tasks if task["assigned_to"] == "writer" and task["status"] == "completed"), None)
...
if __name__ == "__main__":
    ...
    print(f"Starting multi-agent session with ID: {session_id}")
    ...
    print("\n=== Research Findings ===")
    research_message = next((msg.content for msg in result["messages"] if msg.get("agent") == "researcher"), "No research conducted")
    print(research_message)
    
    print("\n=== Written Content ===")
    written_content = next((msg.content for msg in result["messages"] if msg.get("agent") == "writer"), "No content written")
