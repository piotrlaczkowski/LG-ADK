...
class MemoryManager(BaseModel):
    ...
    vector_store: Optional[Any] = Field(None, description="Vector store for semantic search")
    ...
    def add_to_long_term_memory(
        ...
        Args:
            text: The text to store in memory.
    ...
    def search_long_term_memory(self, query: str, k: int = 5) -> List[Dict[str, Any]]:
        """
    ...
    def update_state_with_memory(
        ...
        if "input" in state and state["input"]:
            self.add_message(
                session_id, {"role": "user", "content": state["input"]}
            )
