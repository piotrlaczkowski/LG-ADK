...
class Settings(BaseModel):
    ...
    google_api_key: Optional[SecretStr] = Field(
        None, description="Google API key for Gemini models"
    )
    ollama_base_url: str = Field(
        "http://localhost:11434", description="Ollama API base URL"
    )
    default_llm: str = Field(
        "ollama/llama3", description="Default LLM to use"
    )
    debug: bool = Field(False, description="Debug mode")
    db_url: Optional[str] = Field(
        None, description="Database URL for persistent storage"
    )
    vector_store_path: str = Field(
        "./.vector_store", description="Path to vector store"
    )
    ...
    def from_env(cls) -> "Settings":
        ...
        """
        return cls(
            openai_api_key=SecretStr(os.environ.get("OPENAI_API_KEY", "")) if os.environ.get("OPENAI_API_KEY") else None,
            google_api_key=SecretStr(os.environ.get("GOOGLE_API_KEY", "")) if os.environ.get("GOOGLE_API_KEY") else None,
