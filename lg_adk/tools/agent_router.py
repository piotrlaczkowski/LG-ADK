"""
Agent Router Tool for LG-ADK

This module provides a tool for routing tasks between agents:
1. AgentRouter: For dispatching tasks to the most appropriate agent
"""
...
class AgentRouter:
    ...
    def __init__(self, name: str, agents: List[Agent] = None, 
                ...
                agent_selector: Optional[Callable[[str, List[Agent]], Agent]] = None,
                max_loops: int = 1):
    ...
    def _log(self, level: str, message: str, agent_id: Optional[str] = None, 
    ...
    def _select_agent(self, task: str) -> Agent:
        ...
        for agent in self.agents:
            if (hasattr(agent, "description") and 
                agent.description and 
                any(kw in lower_task for kw in agent.description.lower().split())):
                return agent
            
            if (hasattr(agent, "system_prompt") and 
                agent.system_prompt and 
                any(kw in lower_task for kw in agent.system_prompt.lower().split())):
                return agent
        ...
        for i, agent in enumerate(self.agents):
            ...
            try:
                ...
                self._log("error", f"Agent {agent_name} failed: {str(e)}", 
                         agent_id=agent_name, details={"error": str(e)})
    ...
    async def route_concurrent(self, task: str) -> Dict[str, Any]:
        ...
        for i, agent in enumerate(self.agents):
            agent_name = getattr(agent, "agent_name", f"Agent_{i}")
            self._log("info", f"Adding agent to concurrent pool: {agent_name}", 
                     agent_id=agent_name)
        ...
        for i, result in enumerate(results):
            ...
            if isinstance(result, Exception):
                self._log("error", f"Agent {agent_name} failed: {str(result)}", 
                         agent_id=agent_name, details={"error": str(result)})
        ...
        outputs = [r.get("output", "") for r in combined_result["agent_results"].values() 
    ...
    async def _run_agent_async(self, agent: Agent, input_data: Dict[str, Any], 
    ...
    def _combine_agent_results(self, task: str, 
        ...
        # Set the main output as the concatenation of all outputs
        outputs = [r.get("output", "") for _, r in results if "output" in r]
        combined_result["output"] = "\n\n".join([f"Agent {name}: {r.get('output', '')}" 
                                              for name, r in results if "output" in r])
